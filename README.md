...
Открытая исполняемая спецификация Osnova Reborn. Текущий формат проекта — `0.2`. `0.1` остаётся читаемым и мигрируется только явно.
Каноническая [страница документации](https://github.com/Queryn-labs/osnova-docs) содержит продуктовые и архитектурные пояснения к этим контрактам.

## Статус

Формат проекта `0.2` является текущим контрактом Osnova Reborn. Формат `0.1`
...
- Реализация операций с проектом находится в `osnova-core`, desktop и runtime
  потребляют эти контракты через свои API.
- Папка проекта остаётся источником истины. `.osnova/` содержит производное
  состояние и может быть пересобрана без потери понятности проекта.

## Связанные репозитории

- `osnova-docs` описывает продуктовую и архитектурную позицию.
- `osnova-core` реализует общие TypeScript-пакеты по этим контрактам.
- `osnova-desktop` читает и записывает формат проекта через core API.
- `osnova-runtime` использует формат для проектов, сессий, артефактов и jobs.
- `osnova-plugin-sdk` публикует контракт Extension Manifest v1.
- `osnova-plugins` содержит проверяемые примеры расширений и каталог.
